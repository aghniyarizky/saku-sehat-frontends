...
export default function Pengeluaran({ onSuccess }: PengeluaranProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error("ERROR PENGELUARAN:", teksResponse);
        try {
          const parsedError = JSON.parse(teksResponse);
          throw new Error(parsedError.message || "Gagal menyimpan pengeluaran.");
        } catch {
      ...
      setError(errorObj.message || "Gagal menyimpan pengeluaran.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      {success && (
        <div className="p-3 mb-4 text-sm text-emerald-400 bg-emerald-950/40 border border-emerald-900 rounded-xl backdrop-blur-sm">
          Transaksi pengeluaran berhasil disimpan!
        </div>
      )}
              ...
              <div className="flex flex-col gap-2">
                <div className="px-2">
                  ...
                  <input 
                    ...
                    className="border border-white/15 rounded-full my-1 w-full text-xs px-3 p-1.5 bg-[#101828] focus:outline-none focus:border-[#2EC4B6] transition-colors placeholder:text-white/40 text-white"
                ...
                <div className="px-2">
                  ...
                  <input 
                    ...
                    className="border border-white/15 rounded-full my-1 w-full text-xs px-3 p-1.5 bg-[#101828] focus:outline-none focus:border-[#2EC4B6] transition-colors text-white cursor-pointer"
                  />
                </div>
              </div>
            </div>
