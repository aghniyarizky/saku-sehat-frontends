    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          setFormData((prev) => ({
            ...prev,
            fotoProfilUrl: compressedBase64,
          }));
        };
      };
      reader.readAsDataURL(file);
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        throw new Error(
          "Sesi Anda mungkin sudah habis atau terjadi gangguan pada server. Silakan login ulang.",
        );
      }
      ...
      const updatedData = {
        ...
        sumberPemasukan:
          result.data?.sumberPemasukan || formData.sumberPemasukan,
      };
      ...
      setError(
        err.message ||
          "Terjadi kesalahan saat menyimpan data. Periksa kembali koneksi Anda.",
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, sumberPemasukan: e.target.value })
              }
              className="bg-[#0b0f19] border border-white/10 rounded-4xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#2EC4B6]"
            />
          </div> */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative flex items-center">
              <select
                ...
                className={`w-full bg-[#0b0f19] border border-white/10 rounded-4xl px-4 py-2 text-sm focus:outline-none focus:border-[#2EC4B6] appearance-none cursor-pointer pr-10 ${
                  !formData.sumberPemasukan ? "text-gray-400" : "text-white"
                }`}
              >
                <option
                  value=""
                  disabled
                  hidden
                  className="bg-[#0b0f19] text-gray-400"
                >
